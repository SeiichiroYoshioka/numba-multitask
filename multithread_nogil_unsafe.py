...
def use_numpyapi_nogil(A, B, param):
    A[:, -1] = param
    result = np.zeros(shape=(len(A), len(B)))
...
def main():
    ...
    x = notuse_numpyapi(A, B, 1)
    y = notuse_numpyapi(A, C, 2)
    end_notuse = perf_counter()

    start_use = perf_counter()
    with ThreadPoolExecutor(max_workers=2) as executor:
        future1 = executor.submit(use_numpyapi, A, B, 1)
        future2 = executor.submit(use_numpyapi, A, C, 2)
    x = future1.result()
    y = future2.result()
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        future1 = executor.submit(use_numpyapi_nogil, A, B, 1)
        future2 = executor.submit(use_numpyapi_nogil, A, C, 2)
    x = future1.result()
    y = future2.result()
